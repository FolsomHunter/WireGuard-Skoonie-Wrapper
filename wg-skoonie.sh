...
logDeviceAddedSuccessfullyMessage() {
	...
	msg+="\n"
	msg+="	tools in use on each Linux flavor (traditional ifupdown or Netplan)."
...
outputHelp() {
	...
	msg+="\n"
	msg+="\n"
	msg+="	Interfaces added using this command are configured to allow client devices on the VPN to "
	msg+="\n"
	msg+="	communicate with each other. This is achieved by using WireGuard's PostUp and PostDown "
	msg+="\n"
	msg+="	key-value pairs in the interface's configuration file to modify the server's iptables to "
	msg+="\n"
	msg+="	forward packets from one client to another client, so long as both clients are on the "
	msg+="\n"
	msg+="	same interface and within the same subnet. Clients will not be able to talk to other " 
	msg+="\n"
	msg+="	clients on a different interface/VPN."
	msg+="\n"
	msg+="\n"
	msg+="	If it is preferable that client devices NOT to be able to communicate with each other, it "
	msg+="\n"
	msg+="	is recommended to create an interface per device. This not only prevents the iptables "
	msg+="\n"
	msg+="	rules added by wg-skoonie from allowing client devices to communicate, but it also creates "
	msg+="\n"
	msg+="	another layer of separation between the client devices that may help prevent additional "
	msg+="\n"
	msg+="	security vulnerabilities caused by other iptables rules or services running on the system."
	msg+="\n"
	msg+="\n"
	...
	msg+="\n"
	msg+="	router, ensure that the router is forwarding all traffic for the specified port to the "
	msg+="\n"
	msg+="	server."
	msg+="\n"
	msg+="\n"
	...
	msg+="\n"
	msg+="	port specified in [Server Endpoint] and [Listening Port]. This command does NOT check to "
	msg+="\n"
	msg+="	see if another interface is already listening on the specified port. That is the "
	msg+="\n"
	msg+="	responsibility of the user."
	msg+="\n"
	msg+="\n"
	msg+="	This command does NOT check to see if a previous interface with the same name already exists. "
	msg+="\n"
	msg+="	It is the responsibility of the user to verify this to ensure there are no conflicts."
	msg+="\n"
	msg+="\n"
	...
	msg+="\n"
	msg+="\n"
	msg+="	Adds a new device to the specified interface using the specified IP address."
	msg+="\n"
	msg+="\n"
	msg+="	If the resulting IP address is not within the subnet based on the network details found in"
	msg+="\n"
	msg+="	the wg-skoonie configuration files or if it is already assigned to another device, errors"
	msg+="\n"
	msg+="	are thrown."
	msg+="\n"
	msg+="\n"
