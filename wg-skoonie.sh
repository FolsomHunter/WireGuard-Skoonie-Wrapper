...
readonly VERSION_NUMBER="1.2.1-dev"
...
removeInterface() {
	...
	for ((i = 0; i < ${#filesToDelete[@]}; i++)); do
		...
		if [[ "${filesToDelete[$i]}" == /* ]]; then
			...
			fileToDeleteAbsPath="${WG_SKOONIE_INTERFACES_FOLDER_PATH}/${filesToDelete[$i]}"
		fi
...
showAllInterfacesSkoonie() {
	...
	msg+="\n"
	msg+="\n"
	msg+="	${yellowFontColor}${WG_SKOONIE_INTERFACES_FOLDER_PATH}${resetColors}"
	msg+="\n"
	msg+="\n"
